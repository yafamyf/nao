...
import numpy as np
import datetime
from normalize import normalize_pose_on_shoulders

# Chargement du modèle depuis le fichier joblib
model_dict = joblib.load('modele_action_rf.joblib')
model = model_dict['model'] if isinstance(model_dict, dict) else model_dict
print("✅ Modèle Scikit-learn chargé.")
...
# Liste des indices de landmarks à utiliser
# Indices des 25 landmarks MediaPipe (pour 50 colonnes)
landmark_indices = [
...
with mp_pose.Pose(min_detection_confidence=min_detection_confidence,
    ...
    while opened:
        ...
        if click_detected:
            click_detected = False
            if results.pose_landmarks:
                # Extraction uniquement des landmarks demandés
                coords = []
                ...
                X_scaled = normalize_pose_on_shoulders(X_input)
                ...
                put_text_bottom(image_with_text, f"Classe : {predicted_class}")
